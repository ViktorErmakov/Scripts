...
Процедура Инициализировать()
	...
	///////////////////////////////НастраиваемыеПараметры+++++++++++++++++++++++++++++++++
	
	КаталогTEMP = ПолучитьПеременнуюСреды("TEMP");
	// КаталогTEMP = ПолучитьИмяВременногоФайла("zip");
	// КаталогAPPDATA = ПолучитьПеременнуюСреды("APPDATA");
	
	// Каталог с файлами обновления конфигураций
	Каталоги.Вставить("КаталогФайловОбновления", "D:\_1C_ADD\TMPLTS\1c"); 
	
	// Каталог для хранения dt обновленных версий конфигурации и файлов дампа.
	// Обазятельно в свойствах папки "Безопасность" должен быть полный доступ у того под кем будет выполняться скрипт, я делаю всем
	// иначе файл не запишется
	Каталоги.Вставить("КаталогРезервныхКопий", 	 "D:\_1C_ADD\DemoBaze\"); 
	
	// Параметры подключения к сайту обновлений 1С	
	...
	Каталоги.Вставить("КаталогВременныхФайлов",  ОбъединитьПути(КаталогTEMP, "AutoUpdateIB")); 
	...
	ИнтервалыОжидания.Вставить("ЗавершениеСеансовФайловойИБ", 	1100); // Максимальный интервал ожидания завершения сеанса файловой информационной базы.
...
Процедура ЗапускОбновленияБаз()	
	
	// Бухгалтерия предприятия, редакция 3.0
	///////////////////////////////НастраиваемыеПараметры+++++++++++++++++++++++++++++++++
	ТипКонфигурации = "Accounting";
	ВерсияРелиза = "30";
	НастройкиПодключения.Вставить("ПараметрыПодключенияБП", 
	ПолучитьПараметрыПодключения(,"D:\_1C_BASE\Бухгалтерия Предприятия 3.0\БП 3 демо - типовая",,,,"АбрамовГС (директор)"));
	///////////////////////////////НастраиваемыеПараметры----------------------------------
	
	ПараметрыОбновленияКонфигурации = ПолучитьПараметрыОбновленияКонфигурации(ТипКонфигурации, ВерсияРелиза);
	
	Если ПараметрыОбновленияКонфигурации <> Неопределено Тогда  
		
		РезультатОбновления = ОбновитьИнформационнуюБазу(
		ПараметрыОбновленияКонфигурации, НастройкиПодключения.ПараметрыПодключенияБП, Ложь, , , Ложь, Ложь);
		Если РезультатОбновления Тогда
			Лог.Информация("======================================================================");
			Лог.Информация(СтрШаблон(НСтр("ru = 'Работа с конфигурацей %1, релиз %2, завершена успешно!'"), 
			ТипКонфигурации,  ВерсияРелиза));
			Лог.Информация("======================================================================");
		КонецЕсли;
	...
	ВерсияРелиза = "30";
	НастройкиПодключения.Вставить("ПараметрыПодключенияБПКОПР", 
	ПолучитьПараметрыПодключения(,"D:\_1C_BASE\Бухгалтерия Предприятия 3.0\БП 3 Корп Демо - типовая (Обновляемая)",,,,"admin"));
	///////////////////////////////НастраиваемыеПараметры----------------------------------
	...
	Если ПараметрыОбновленияКонфигурации <> Неопределено Тогда  
		...
		РезультатОбновления = ОбновитьИнформационнуюБазу(
		ПараметрыОбновленияКонфигурации, НастройкиПодключения.ПараметрыПодключенияБПКОПР, Ложь, , , Ложь, Ложь);
		Если РезультатОбновления Тогда
			Лог.Информация("======================================================================");
			Лог.Информация(СтрШаблон(НСтр("ru = 'Работа с конфигурацей %1, релиз %2, завершена успешно!'"), 
			ТипКонфигурации,  ВерсияРелиза));
	...
	ТипКонфигурации = "Accounting";
	ВерсияРелиза = "20";
	НастройкиПодключения.Вставить("ПараметрыПодключенияБП2", 
	ПолучитьПараметрыПодключения(,"D:\_1C_BASE\Бухгалтерия Предприятия 2.0\БП 2 Демо - типовая (Обновляемая)",,,,"Любимов (администратор)"));
	///////////////////////////////НастраиваемыеПараметры----------------------------------
	...
	Если ПараметрыОбновленияКонфигурации <> Неопределено Тогда  
		...
		РезультатОбновления = ОбновитьИнформационнуюБазу(
		ПараметрыОбновленияКонфигурации, НастройкиПодключения.ПараметрыПодключенияБП2, Ложь, , , Ложь, Ложь);
		Если РезультатОбновления Тогда
			Лог.Информация("======================================================================");
			Лог.Информация(СтрШаблон(НСтр("ru = 'Работа с конфигурацей %1, релиз %2, завершена успешно! %3'"), 
			ТипКонфигурации,  ВерсияРелиза, Формат(ТекущаяДата(), "ДЛФ=T")));
			Лог.Информация("======================================================================");
		КонецЕсли;
	...
	///////////////////////////////НастраиваемыеПараметры+++++++++++++++++++++++++++++++++
	ТипКонфигурации = "Trade";
	ВерсияРелиза = "114";
	НастройкиПодключения.Вставить("ПараметрыПодключенияУТ11", 
	ПолучитьПараметрыПодключения(,"D:\_1C_BASE\Управление торговлей 11.3\УТ (демо) 11.3",,,,"Администратор (ОрловАВ)"));
	///////////////////////////////НастраиваемыеПараметры----------------------------------
	...
	Если ПараметрыОбновленияКонфигурации <> Неопределено Тогда  
		...
		РезультатОбновления = ОбновитьИнформационнуюБазу(
		ПараметрыОбновленияКонфигурации, НастройкиПодключения.ПараметрыПодключенияУТ11, Ложь, , , Ложь, Ложь);
		Если РезультатОбновления Тогда
			Лог.Информация("======================================================================");
			Лог.Информация(СтрШаблон(НСтр("ru = 'Работа с конфигурацей %1, релиз %2, завершена успешно!'"), 
			ТипКонфигурации,  ВерсияРелиза));
	...
	ТипКонфигурации = "Trade";
	ВерсияРелиза = "103";
	НастройкиПодключения.Вставить("ПараметрыПодключенияУТ10", 
	ПолучитьПараметрыПодключения(,"D:\_1C_BASE\Управление Торговлей 10.3\УТ 10.3 ПРОФ",,,,"Федоров (администратор)"));
	///////////////////////////////НастраиваемыеПараметры----------------------------------
	...
	Если ПараметрыОбновленияКонфигурации <> Неопределено Тогда  
		...
		РезультатОбновления = ОбновитьИнформационнуюБазу(
		ПараметрыОбновленияКонфигурации, НастройкиПодключения.ПараметрыПодключенияУТ10, Ложь, , , Ложь, Ложь);
		Если РезультатОбновления Тогда
			Лог.Информация("======================================================================");
			Лог.Информация(СтрШаблон(НСтр("ru = 'Работа с конфигурацей %1, релиз %2, завершена успешно!'"), 
			ТипКонфигурации,  ВерсияРелиза));
	...
	// Розница 2.0
	///////////////////////////////НастраиваемыеПараметры+++++++++++++++++++++++++++++++++
	ТипКонфигурации = "Retail";
	ВерсияРелиза = "22";
	НастройкиПодключения.Вставить("ПараметрыПодключенияРозница", 
	ПолучитьПараметрыПодключения(,"D:\_1C_BASE\Розница\Розница (демо)",,,,"БорисовФМ (Администратор)"));
	///////////////////////////////НастраиваемыеПараметры----------------------------------
	...
	Если ПараметрыОбновленияКонфигурации <> Неопределено Тогда  
		...
		РезультатОбновления = ОбновитьИнформационнуюБазу(
		ПараметрыОбновленияКонфигурации, НастройкиПодключения.ПараметрыПодключенияРозница, Ложь, , , Ложь, Ложь);
		Если РезультатОбновления Тогда
			
			// Делаем еще одну резервную копию, что бы актуальная версия демо базы была в архиве.
			РезультатАрхивирования = СоздатьРезервнуюКопию(
				НастройкиПодключения.ПараметрыПодключенияРозница, 
				ПараметрыОбновленияКонфигурации.ТипКонфигурации, 
				ПараметрыОбновленияКонфигурации.ВерсияРелиза);
	...
	///////////////////////////////НастраиваемыеПараметры+++++++++++++++++++++++++++++++++
	ТипКонфигурации = "Enterprise";
	ВерсияРелиза = "13";
	НастройкиПодключения.Вставить("ПараметрыПодключенияУПП", 
	ПолучитьПараметрыПодключения(,"D:\_1C_BASE\Управление Производственным Предприятием\УПП 1.3 Демо - типовая (Обновляемая)",,,,"Любимов (администратор)"));
	///////////////////////////////НастраиваемыеПараметры----------------------------------
	...
	Если ПараметрыОбновленияКонфигурации <> Неопределено Тогда  
		...
		РезультатОбновления = ОбновитьИнформационнуюБазу(
		ПараметрыОбновленияКонфигурации, НастройкиПодключения.ПараметрыПодключенияУПП, Ложь, , , Ложь, Ложь);
		Если РезультатОбновления Тогда
			Лог.Информация("======================================================================");
			Лог.Информация(СтрШаблон(НСтр("ru = 'Работа с конфигурацей %1, релиз %2, завершена успешно! %3'"), 
			ТипКонфигурации,  ВерсияРелиза, Формат(ТекущаяДата(), "ДЛФ=T")));
	...
	// ERP УправлениеПредприятием 2.0
	///////////////////////////////НастраиваемыеПараметры+++++++++++++++++++++++++++++++++
	ТипКонфигурации = "Enterprise20";
	ВерсияРелиза = "22";
	НастройкиПодключения.Вставить("ПараметрыПодключенияERP", 
	ПолучитьПараметрыПодключения(,"D:\_1C_BASE\ERP\ERP 2 Демо - типовая (Обновляемая)",,,,"Администратор (ОрловАВ)"));
	///////////////////////////////НастраиваемыеПараметры----------------------------------
	...
	Если ПараметрыОбновленияКонфигурации <> Неопределено Тогда  
		...
		РезультатОбновления = ОбновитьИнформационнуюБазу(
		ПараметрыОбновленияКонфигурации, НастройкиПодключения.ПараметрыПодключенияERP, Ложь, , , Ложь, Ложь);
		Если РезультатОбновления Тогда
			Лог.Информация("======================================================================");
			Лог.Информация(СтрШаблон(НСтр("ru = 'Работа с конфигурацей %1, релиз %2, завершена успешно!'"), 
			ТипКонфигурации,  ВерсияРелиза));
	...
	// Документооборот 1.4
	///////////////////////////////НастраиваемыеПараметры+++++++++++++++++++++++++++++++++
	ТипКонфигурации = "DocMng";
	ВерсияРелиза = "14";
	НастройкиПодключения.Вставить("ПараметрыПодключенияДО", 
	ПолучитьПараметрыПодключения(,"D:\_1C_BASE\Документооборот\ДО 1.4 Проф Демо - типовая (Обновляемая)",,,,"Администратор"));
	///////////////////////////////НастраиваемыеПараметры----------------------------------
	...
	Если ПараметрыОбновленияКонфигурации <> Неопределено Тогда  
		...
		РезультатОбновления = ОбновитьИнформационнуюБазу(
		ПараметрыОбновленияКонфигурации, НастройкиПодключения.ПараметрыПодключенияДО, Ложь, , , Ложь, Ложь);
		Если РезультатОбновления Тогда
			Лог.Информация("======================================================================");
			Лог.Информация(СтрШаблон(НСтр("ru = 'Работа с конфигурацей %1, релиз %2, завершена успешно!'"), 
			ТипКонфигурации,  ВерсияРелиза));
...
//
Функция ПолучитьПараметрыОбновленияКонфигурации(ТипКонфигурации = "Accounting", ВерсияРелиза = "30", ВерсияПлатформы = "83")
	
	Лог.Информация("======================================================================");
	Лог.Информация(СтрШаблон(НСтр("ru = 'Конфигурация %1, релиз %2, платформа %3. %4'"), 
	ТипКонфигурации,  ВерсияРелиза,  ВерсияПлатформы, Формат(ТекущаяДата(), "ДЛФ=T")));     
	Лог.Информация("======================================================================");
...
Функция ПолучитьСпискиОбновленияКонфигурациии(ПараметрыОбновленияКонфигурации)
	...
	Попытка
		...
		Соединение = Новый HTTPСоединение(НастройкиПодключения.СерверПроверкиОбновлений, , Аутентификация.Пользователь, Аутентификация.Пароль, Новый ИнтернетПрокси(Истина));
	Исключение
...
Функция ОбновитьИнформационнуюБазу(ПараметрыОбновленияКонфигурации, ПараметрыПодключения,
	...
	Пока Истина Цикл
		...
		Если СоздаватьРезервнуюКопию Тогда 	
			РезультатВыполнения = СоздатьРезервнуюКопию(ПараметрыПодключения, 
			ПараметрыОбновленияКонфигурации.ТипКонфигурации, 
			ПараметрыОбновленияКонфигурации.ВерсияРелиза);
			Если Не РезультатВыполнения Тогда
				Прервать;
		...
		Если Не ВерсияДляОбновления = "КонфигурацияИзменена" Тогда 
			...
			Если ВыполнитьОтложенныеОбработчики Тогда 	
				РезультатВыполнения = ВыполнитьОтложенныеОбработчикиОбновления(ПараметрыПодключения, ПараметрыОбновленияКонфигурации);
				Если Не РезультатВыполнения Тогда
					Прервать;
				КонецЕсли;
			КонецЕсли;
		...
		РезультатВыполнения = ПринятьОбновленияВИнформационнойБазе(ПараметрыПодключения, ПараметрыОбновленияКонфигурации);
		Если Не РезультатВыполнения Тогда
			Прервать;
		КонецЕсли;
...
Функция ПроверитьНаличиеОбновлений(ПараметрыОбновленияКонфигурации, ПараметрыПодключения)
	...
	ПараметрыПодключения.Вставить("ТекущаяВерсияКонфигурацииИнформационнойБазы", ТекущаяВерсия);
	...
	Если КонфигурацияИзменена Тогда
		Лог.Информация(НСтр("ru = 'Основная конфигурация отличается от конфигурации постащика.'"));
...
Функция ПолучитьФайлыОбновлений(ВерсияДляОбновления) 
	...
	Попытка
		...
		Соединение = Новый HTTPСоединение(НастройкиПодключения.СерверОбновлений, , Аутентификация.Пользователь, Аутентификация.Пароль, Новый ИнтернетПрокси(Истина));
	Исключение
		Лог.Ошибка(СтрШаблон(НСтр("ru = 'Не удалось установить HTTP-соединение с сервером %1:
...
//  Булево - Признак успешного выполнения.
//
Функция СоздатьРезервнуюКопию(ПараметрыПодключения, ТипКонфигурации, ВерсияРелиза)
	...
	//ИмяФайла = "Base" + Формат(ТекущаяДата(), "ДФ=ггММддЧЧммсс");//Ермаков
	МассивТипВерсия = Новый Массив();
	МассивТипВерсия.Добавить(ТипКонфигурации);
	МассивТипВерсия.Добавить(ВерсияРелиза);
	ТипВерсияКонфигурации = СтрСоединить(МассивТипВерсия, "_");
	ИмяФайла = ОбъединитьПути(Каталоги.КаталогРезервныхКопий, ТипВерсияКонфигурации);
	...
	Если ФайловыйВариантРаботы Тогда  
		...
		//Проверяет есть ли каталог для резервных копий конфигурации.
		КаталогКопии = Новый Файл(ИмяФайла);
		Если Не КаталогКопии.Существует() Тогда
			Попытка
				СоздатьКаталог(ИмяФайла);
			Исключение
				Лог.Ошибка(СтрШаблон(НСтр("ru = 'Ошибка при создании каталога резервных копий.
				|%1'"), ИнформацияОбОшибке()));
			КонецПопытки;
		КонецЕсли;
		
		ПолноеИмяФайла = ОбъединитьПути(ИмяФайла, ПараметрыПодключения.ТекущаяВерсияКонфигурацииИнформационнойБазы + ".zip");
		// Проверяем есть ли уже такой файл архива
		ФайлАрхива = Новый Файл(ПолноеИмяФайла);
		Если ФайлАрхива.Существует() Тогда
			Лог.Информация(НСтр("ru = 'Файл с архивом этой конфигурации уже есть.'"));
			Возврат Истина;
		КонецЕсли;
...
//  Булево - Признак успешного выполнения.
//
Функция ВыполнитьОтложенныеОбработчикиОбновления(ПараметрыПодключения, ПараметрыОбновленияКонфигурации)
	...
	Если (ПараметрыОбновленияКонфигурации.ТипКонфигурации = "Accounting"
		И ПараметрыОбновленияКонфигурации.ВерсияРелиза = "20")
		ИЛИ (ПараметрыОбновленияКонфигурации.ТипКонфигурации = "Enterprise"
		И ПараметрыОбновленияКонфигурации.ВерсияРелиза = "13")
		ИЛИ (ПараметрыОбновленияКонфигурации.ТипКонфигурации = "Trade"
		И ПараметрыОбновленияКонфигурации.ВерсияРелиза = "103") Тогда
		Лог.Информация(НСтр("ru = 'Выполнение отложенных обработчиков обновления в этой конфигурации не возможно.'"));
		ОсвободитьОбъект(Соединение);
		ВыполнитьСборкуМусора();
		Соединение = Неопределено;
		ОжидатьЗавершения(ПараметрыПодключения);
...
//  Булево - Признак успешного выполнения.
//
Функция ПринятьОбновленияВИнформационнойБазе(ПараметрыПодключения, ПараметрыОбновленияКонфигурации)
	...
	Попытка
		// Если это бп 20 тогда передавать параметр не нужно!
		Если (ПараметрыОбновленияКонфигурации.ТипКонфигурации = "Accounting"
			И ПараметрыОбновленияКонфигурации.ВерсияРелиза = "20")
			ИЛИ (ПараметрыОбновленияКонфигурации.ТипКонфигурации = "Enterprise"
			И ПараметрыОбновленияКонфигурации.ВерсияРелиза = "13") Тогда
			Соединение.ОбновлениеИнформационнойБазы.ВыполнитьОбновлениеИнформационнойБазы();
			Результат = "Успешно";
		ИначеЕсли ПараметрыОбновленияКонфигурации.ТипКонфигурации = "DocMng" Тогда
			Результат = Соединение.ОбновлениеИнформационнойБазыВызовСервера.ВыполнитьОбновлениеИнформационнойБазы(Ложь);
		Иначе
...
Процедура ОтправитьПисьмоСЛогомНаПочту(НаименованиеФайлаЛог)//Ермаков
	...
	Сообщение.Получатели.Добавить("");
	//Сообщение.ОбратныйАдрес.Добавить("");
	Сообщение.Отправитель = "";
	Сообщение.Тема        = "Автообновление типовых конфигураций";
	Текст =  "Во вложении файл лога обновлений";
	
	Сообщение.Тексты.Добавить(Текст, ТипТекстаПочтовогоСообщения.HTML);
	Сообщение.Вложения.Добавить(НаименованиеФайлаЛог);
	//Сообщение.Вложения.Добавить(Новый ДвоичныеДанные("C:/Пример вложения 2.docx"), "Пример вложения 2.docx");
...
НаименованиеФайлаЛог = СтрШаблон("D:\_1C_ADD\DemoBaze\Obnovlenie_%1.log", Формат(ТекущаяДата(), "ДФ=ггггММдд"));
//НаименованиеФайлаЛог = СтрШаблон("D:\1С\_1C_ADD\DemoBazeObnovlenie_%1.log", Формат(ТекущаяДата(), "ДФ=ггггММддЧЧмм"));
ФайлЖурнала = Новый ВыводЛогаВФайл;
ФайлЖурнала.ОткрытьФайл(НаименованиеФайлаЛог);
Лог.ДобавитьСпособВывода(ФайлЖурнала);

Инициализировать();
Лог.Закрыть();
ОтправитьПисьмоСЛогомНаПочту(НаименованиеФайлаЛог);
